...
export function KnowledgeHero() {
  const t = useTranslations('KnowledgePage.Hero');

  return (
    <section className="relative w-full min-h-[calc(100vh-88px)] flex flex-col justify-center py-16 lg:py-20 bg-background overflow-hidden" aria-labelledby="knowledge-heading">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-12 relative z-10">
        <div className="max-w-7xl mx-auto">
          ...
          <header className="relative mb-12 lg:mb-20">
            <h1 id="knowledge-heading" className="font-serif text-[12vw] sm:text-[13vw] lg:text-[8rem] font-medium leading-[0.9] tracking-tighter text-foreground motion-safe:animate-slide-up">
              {t('mainTitlePart1')} <br />
              {t('mainTitlePart2')} <br />
              <span className="text-muted-foreground/40 italic font-light">{t('mainTitlePart3')}</span>
            </h1>
          </header>
            ...
            <div className="lg:col-span-5 flex flex-col space-y-6 sm:space-y-8 items-start lg:items-end w-full">
              ...
              <div className="flex flex-wrap gap-x-4 sm:gap-x-8 gap-y-3 sm:gap-y-4 text-[9px] font-black tracking-[0.3em] sm:tracking-[0.4em] text-muted-foreground/30 uppercase w-full justify-start lg:justify-end">
                <button
                  ...
                  aria-label="View arrival process guides"
                  tabIndex={0}
                >
                  {t('quickLinks.arrival')}
                </button>
                <button
                  className="hover:text-danish-red transition-colors cursor-pointer min-h-[44px] flex items-center bg-transparent border-0 p-0 focus-visible:text-danish-red focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-danish-red focus-visible:ring-offset-2"
                  aria-label="View housing guides"
                  tabIndex={0}
                >
                ...
                <button
                  className="hover:text-danish-red transition-colors cursor-pointer min-h-[44px] flex items-center bg-transparent border-0 p-0 focus-visible:text-danish-red focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-danish-red focus-visible:ring-offset-2"
                  aria-label="View benefits guides"
                  tabIndex={0}
                >
