...
export function GuidanceHero() {
  const t = useTranslations('GuidancePage.Hero');

  return (
    <section className="relative w-full min-h-[calc(100vh-88px)] flex flex-col justify-center py-16 lg:py-20 bg-background overflow-hidden" aria-labelledby="guidance-heading">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-12 relative z-10">
        <div className="max-w-7xl mx-auto">
          ...
          <header className="relative mb-12 lg:mb-20">
            <h1 id="guidance-heading" className="font-serif text-[12vw] sm:text-[13vw] lg:text-[8rem] font-medium leading-[0.9] tracking-tighter text-foreground motion-safe:animate-slide-up">
              {t('title')} <br />
              {t('titleWith')} <br />
              <span className="text-muted-foreground/40 italic font-light">{t('titleConfidence')}</span>
            </h1>
          </header>
          ...
          <div className="grid lg:grid-cols-12 gap-8 sm:gap-12 lg:gap-24 items-start pt-8 sm:pt-12 border-t border-border/40">
            <div className="lg:col-span-7 space-y-4 sm:space-y-6">
              ...
              <p className="text-lg sm:text-xl md:text-3xl lg:text-4xl text-foreground/90 font-serif font-light leading-[1.2] sm:leading-[1.15] max-w-2xl motion-safe:animate-fade-in delay-200">
                {t('description')} <span className="italic font-normal">{t('descriptionEmphasis')}</span>.
              </p>
            </div>
            ...
            <div className="lg:col-span-5 flex flex-col space-y-6 sm:space-y-8 items-start lg:items-end w-full">
              ...
              <div className="flex flex-wrap gap-x-4 sm:gap-x-8 gap-y-3 sm:gap-y-4 text-[9px] font-black tracking-[0.3em] sm:tracking-[0.4em] text-muted-foreground/30 uppercase w-full justify-start lg:justify-end">
                <button
                  ...
                  aria-label={`View ${t('categories.arrival').toLowerCase()} guidance`}
                  tabIndex={0}
                >
                  {t('categories.arrival')}
                </button>
                <button
                  className="hover:text-danish-red transition-colors cursor-pointer min-h-[44px] flex items-center bg-transparent border-0 p-0 focus-visible:text-danish-red focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-danish-red focus-visible:ring-offset-2"
                  aria-label={`View ${t('categories.setup').toLowerCase()} guidance`}
                  tabIndex={0}
                >
                  {t('categories.setup')}
                </button>
                <button
                  className="hover:text-danish-red transition-colors cursor-pointer min-h-[44px] flex items-center bg-transparent border-0 p-0 focus-visible:text-danish-red focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-danish-red focus-visible:ring-offset-2"
                  aria-label={`View ${t('categories.housing').toLowerCase()} guidance`}
                  tabIndex={0}
                >
                ...
                <button
                  className="hover:text-danish-red transition-colors cursor-pointer min-h-[44px] flex items-center bg-transparent border-0 p-0 focus-visible:text-danish-red focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-danish-red focus-visible:ring-offset-2"
                  aria-label={`View ${t('categories.taxes').toLowerCase()} guidance`}
                  tabIndex={0}
                >
                  {t('categories.taxes')}
                </button>
